...
stack<int> st; //ノードを入れるキュー
stack<int> st_num_step_count; // ノードに来るまでに何ステップかかるか
vector<bool> visited(MAX_ID); // 訪ねるノードをマーク
vector<int> prev_node(MAX_ID, -1); //　prev(id):idの前に訪れたノードのid

int dfs(int id_node, int id_start, int id_target, int num_step_count) {
    /*
    ...
    */
    ...
    st.push(id_start);
    st_num_step_count.push(0);

    while (!st.empty()) {

        id_node = st.top(); //これから調べるノードID(int)
        st.pop();

        num_step_count = st_num_step_count.top(); // id_nodeにたどり着くまでに何回かかるか
        st_num_step_count.pop();
        ...
        if (links.count(id_node) > 0) {
            ...
            for (auto itr = set_id.begin(); itr != set_id.end(); ++itr) {
                ...
                if (!visited.at(id_next)) {
                    ...
                    if (id_node != id_next) {
                        st.push(id_next); // 次に訪れるノードをキューにプッシュ
                        st_num_step_count.push(num_step_count + 1); // 何回かかったか
...
int main() {
    ...
    //bfs関数で計算します。
    int ans = dfs(id_start, id_start, id_target, 0);
